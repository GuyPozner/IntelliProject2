...
sessionPath <- "Sessions/2016-06-05 10.40.56/"
...
source("findCorrectSide.R")
windowNosepokes$CorrectSide <- lapply(windowNosepokes$VisitID,function(x){
                                    findCorrectSide(
                                        windowNosepokes$Side[windowNosepokes$VisitID == x], windowNosepokes$ModuleName[windowNosepokes$VisitID == x])})
windowNosepokes$SideError <- FALSE
windowNosepokes$SideError[(windowNosepokes$TimeError == 0) & (windowNosepokes$CorrectSide == FALSE)] <- TRUE
source("sumTimeErrors.R")
source("sumSideErrors.R")
source("sumNosepokes.R")
TimeError <- as.numeric()
Count <- as.numeric()
SideError <- as.numeric()
for(i in 1:nrow(results)){
        TimeError[i] <- sumTimeErrors(windowNosepokes, 
                                      results$AnimalName[i], 
                                      results$Day[i], 
                                      results$ModuleName[i])
        SideError[i] <- sumSideErrors(windowNosepokes, 
                                      results$AnimalName[i], 
                                      results$Day[i], 
                                      results$ModuleName[i])
...
p6 <- ggplot(windowNosepokes, aes(ReactTime)) +
        geom_histogram(binwidth = 0.1) +
        scale_x_continuous(breaks = seq(0,25,1)) +
        ggtitle("Reaction times histogram") +
        geom_vline(xintercept =  median(windowNosepokes$ReactTime),
